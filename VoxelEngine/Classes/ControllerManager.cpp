...
const float Controller::convertAxisValue(ControllerID rawValue, const float modifier)
{
	float value = 0;
...
ControllerManager::ControllerManager()
{
	...
	{
		cout << "\n[ControllerManager] SDL_INIT_GAMECONTROLLER or SDL_INIT_HAPTIC could not initialize! SDL Error : " << SDL_GetError() << "\n" << endl;
		active = false;
	}
	else
	{
		cout << "\n[ControllerManager] SDL2 is initilized with GAMECONTROLLER and HAPTIC\n" << endl;
		active = true;
	}
}
...
float Voxel::ControllerManager::getAxisValue(ControllerID id, IO::XBOX_360::AXIS axis)
{
	Controller* controller = findController(id);
	if (controller != nullptr)
	{
		return controller->axisValueMap[axis];
	}
	else
	{
		return 0.0f;
