...
Voxel::UI::FadeTo2::FadeTo2()
	: Action()
	, opacity(0.0f)
	, target(nullptr)
{}

bool Voxel::UI::FadeTo2::init(const float duration, const float opacity)
{
	if (duration < 0.0f || opacity < 0.0f || opacity > 1.0f)
	{
		return false;
...
Voxel::UI::FadeTo2 * Voxel::UI::FadeTo2::create(const float duration, const float opacity)
{
	auto newFadeTo = new FadeTo2();
	if (newFadeTo->init(duration, opacity))
	{
		return newFadeTo;
	}
...
void Voxel::UI::FadeTo2::setTarget(TransformNode * target)
{
	if (target)
	{
		this->target = target;
...
void Voxel::UI::FadeTo2::update(const float delta)
{
	elapsedTime += delta;
	//std::cout << "FadeTo: " << elapsedTime << " / " << duration << "\n";
